...
public interface GoodsRepository extends JpaRepository<GoodsEntity, Long> {
    ...
    @Query(value = "SELECT DATE(created_at) as date, COUNT(*) as count FROM goods WHERE DATE(created_at) >= CURDATE() - INTERVAL 6 DAY AND DATE(created_at) <= CURDATE() GROUP BY DATE(created_at)", nativeQuery = true)
